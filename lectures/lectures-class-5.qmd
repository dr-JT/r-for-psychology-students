...
In [Class 4](lectures/lectures-class-4.html), we saw that the first stage in the data processing workflow is **data preparation**. This is a critical, though often overlooked stepped. This class will focus on some general principals and common data preparation steps.
...
3.  (Optional) Rename the data file to something shorter (e.g., rawdata_messy.csv)
...
However, if you only have one raw data file because your data is all collected in a single task/measure then you can just create one **tidyraw** script
:::
...
data_import <- 
  data.frame(ID = c(rep(1, 9), 
             ...
             slide2.rt = c(rep(NA, 4), 431,223,643,742,667, 
                           rep(NA, 4), 455,743,456,826,342))
...
Finally, you just need to save this tidy data frame to a .csv file in your **data/raw** folder and use the name of the task/measure that it came from.
