...
1.  Using `setwd()`
2.  Using the RStudio Import Dataset GUI
3.  Using RProjects and `here()`
...
-   Paste the code into your **Untitled.R** script
    ...
    -   The most useful thing here wil be the **absolute** file path.
...
Tab-delimited files are typically saved with the more standard **.txt** file extension (e.g., datafile.txt) but a lot of other file formats might also have the **.txt** extension. This can create some ambiguity as to how the data is stored and formatted for .txt files.
:::
...
**Long**: Variables and values are stacked in rows, making the data frame longer
...
-   This often results in:
    ...
    -   Between-subject variables in wide format

    -   Within-subject variables in long format
...
Restructuring data involves changing the structure from long-to-wide or wide-to-long. The `tidyr` package provides useful functions to do this:
...
R does not like column names to have spaces in them, but it does allow it. Notice how in `data_wide` the column `Stress Level` contains a space. This is because the value in `data_long` had a space, which was not a problem then.
...
data_wide <- clean_names(data_wide, case = "parse")
```
:::
...
*Note that this activity was created with the assistance of ChatGPT 4.*
...
You're analyzing data from a psychology experiment where participants' cognitive performance scores and mood ratings are recorded across four weeks for two interventions. The data is split into two files, one with the cognitive performance data ([**class_2_cog_data.csv**](/data/class_2_cog_data.csv)) and another with the mood ratings data ([**class_2_mood_data.txt**](/data/class_2_mood_data.txt)), but the same participants across each data set.

Your task will involve importing, restructuring, and merging the data so that we can graphically visualize the results of the experiment.
...
5.  Restructure each data set to long format with `week` spread across rows instead of columns and one column for `cognitive score` or `mood rating`
    ...
    -   You will notice that the values in the `week` column also contain the string: `cognitive_week_` or `mood_week_`. This is redundant information with the column names and can be removed to clean up the format of the data. The following line of code can be used to to do so. **This is an optional step.**
...
7.  Plot the cognitive performance data:
    ...
    1.  First add the data and aesthetic layers - how our data map onto the plot scales and axes
...
9.  Save the plots as a file on your computer
    ...
    -   Place the following code directly below the code for creating each of the plots. It will save the last generated plot to a file. Change the file name for each plot.
        ...
        ggsave(here("folder/path/class_2_cognitive_plot.png"), 
...
10. Organize and clean up your script
    ...
    1.  Load all packages at the top of your script
    2.  Get rid of code that is not a necessary step for creating the plots
    3.  Add comment headers to sections of your code: e.g., `# load packages`
...
ggsave(here("~/Users/jtsukahara3/GitHub Repos/r-for-psychology-students/lectures/images/class_2_cognitive_plot.png"), 
       width = 6, height = 4, dpi = 300)
...
ggsave(here("~/Users/jtsukahara3/GitHub Repos/r-for-psychology-students/lectures/images/class_2_mood_plot.png"), 
       width = 6, height = 4, dpi = 300)
