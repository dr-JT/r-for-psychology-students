...
If you did not do so already... download the sample data files

-   a_file.csv

-   another_file.txt

Save them wherever. Open up two R scripts and save them wherever

-   Untitled.R: scratchpad R script

-   Class_2.R: reproducible script file
...
1.  Using `setwd()`
2.  Using the RStudio GUI Import Dataset window
3.  Using RProjects and `here()`

Never... ever... ever... use option 1, `setwd()`.

Instead, you should always use RProjects and `here()`. But we will not cover this until Class 5. For now, we can just use the RStudio GUI Import Dataset window to find the **absolute** file path.

### RStudio GUI

When you are having difficulty importing a file correctly or unsure of the file format the RStudio Import GUI can be really useful.
...
In the **Class_2.R** script, import a_file.csv.

When you import a file for the first time it is always a good idea to view it and spend some time getting to know it. Based on what you learned in [Class 1](/lectures/lectures-class-1.html), use `View()`, `head()`, `colnames()`, and/or `unique()` to better understand this data set.
...
Whenever joining, you need to specify what are the **key** column(s) to join by - columns that are common between the data frames.
